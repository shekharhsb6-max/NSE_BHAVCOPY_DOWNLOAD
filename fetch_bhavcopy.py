...
def write_date_batch(
    ...
    # Google Sheets has a finite row grid.  The worksheet may contain
    # exactly as many rows as the current data, so a normal range write can
    # fail when a new trading day is appended.  Expand the grid first.
    current_row_count = worksheet.row_count

    if current_row_count < required_last_row:
        ...
        print(
            f"Expanding RAW_DATA by {rows_to_add} rows."
        )
    ...
    worksheet.update(
        f"A{next_row}:O{required_last_row}",
        rows,
        value_input_option="USER_ENTERED",
    )
